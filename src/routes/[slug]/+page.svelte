...
<div class="flex flex-col items-center justify-between w-screen h-screen max-h-screen">
	{#if data.meta.type === 'content'}
		<section class="flex flex-col justify-center w-5/6 h-full pt-10 mx-auto">
				...
				<hgroup class="flex items-center w-full mb-10">
					<h1 class="w-full italic font-black text-center text-yellow-400 text-7xl">{data.meta.title}</h1>
				</hgroup>
	...
	{:else if data.meta.type === 'demo'}
		<section class="w-5/6 h-full pt-10 mx-auto">
			...
			<div class="flex flex-col items-center justify-center gap-4 p-4 overflow-auto border border-gray-800 overflow rounded-2xl max-h-[80vh]">
				<svelte:component this={data.content} />
			</div>
		</section>
